...
class LocationControllerIntegrationTest {
    ...
    void createLocation_zeroCapacity_returnsUnprocessableEntity() {
        ...
        webTestClient.post()
                ...
                .bodyValue(model)
                .exchange()
                .expectStatus().isCreated(); // Depends on your logic — change to isEqualTo(HttpStatus.UNPROCESSABLE_ENTITY) if it's invalid.
    }
    ...
    @Test
    void postLocation_withMalformedJson_shouldReturnBadRequest() {
        String brokenJson = "{\"name\":\"Bad JSON\" \"shelterType\":\"SHELTER\"}"; // missing comma

        webTestClient.post()
                .uri(BASE_URI)
                .contentType(MediaType.APPLICATION_JSON)
                .bodyValue(brokenJson)
                .exchange()
