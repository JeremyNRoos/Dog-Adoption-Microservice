...
API_FIELDS["http://localhost:8080/api/v1/volunteers"]="email"
API_FIELDS["http://localhost:8080/api/v1/locations/3e6c62a3-b1e7-4eb7-9642-8cdcb3ac74e6/dogs"]="name"
API_FIELDS["http://localhost:8080/api/v1/adopters/fcbf86b1-8a76-4d2b-a352-75b10a8fd4a1/adoptionpapers"]=""
...
ID_KEY_FROM_URL() {
  ...
  [[ "$endpoint" == "adoptionpapers" ]] && echo "adoptionPaperId" || echo "${endpoint%s}Id"
}
...
randomize_fields() {
  ...
  for field in "${fields[@]}"; do
    ...
    elif [[ "$field" == "adoptionPaperId" ]]; then
      value="p1"  # This is the fixed ID for adoptionPaperId
    else
...
test_crud_operations() {
  ...
  if [[ "$base_url" == *"adoptionpapers" ]]; then
      local get_id=$(echo "$updated_payload" | jq -r ".${id_key}")
      assertEqual "$get_id" "$new_id" "adoptionPaperId PUT/GET match"
  fi
  # DELETE
