...
public class DogServiceClient {
    ...
    public DogResponseModel getDogById(String locationId, String dogId) {
        try {
            ...
            DogResponseModel dogResponseModel = restTemplate.getForObject(url, DogResponseModel.class);
    ...
    public List<DogResponseModel> getAllDogs(String locationId) {
        try {
            ...
            ResponseEntity<List<DogResponseModel>> response = restTemplate.exchange(url, HttpMethod.GET, null, new ParameterizedTypeReference<List<DogResponseModel>>() {});
    ...
    public DogResponseModel addDog(String locationId,DogRequestModel dogRequestModel) {
        try{
            String url = DOGS_SERVICE_BASE_URL + "/" + locationId + "/" + "dogs" ;
            log.debug("Dogs-Service URL is: " + url);
            DogResponseModel dogResponseModel = restTemplate.postForObject(url, dogRequestModel, DogResponseModel.class);
            return dogResponseModel;
        }
        catch (HttpClientErrorException ex) {
            throw handleHttpClientException(ex);
